...
Gerado de [fontes.json](fontes.json) em 2026-09-23. 43 fontes/famílias; não é uma contagem de APIs nem de fornecedores aprovados. Não editar esta tabela separadamente: atualizar o JSON e executar `consultar-fontes.py --gerar-indice`.

Todas as ligações remetem a fontes primárias. Para nível de evidência, amostras, data e próxima ação por fonte, consultar o JSON pelo ID. A primeira rodada é de 22/09; esta expansão é de 23/09. Páginas bloqueadas ou apenas indexadas estão identificadas no JSON.
...
| **PR05** · [NHS exercícios de força, sentados, equilíbrio e flexibilidade](https://www.nhs.uk/live-well/exercise/strength-exercises/) | candidato | Texto elegível sob OGL com condições NHS. Tradução é adaptação; atribuição específica e exceções de mídia devem ser respeitadas. | Candidato a educação e rotinas introdutórias, não biblioteca de hipertrofia periodizada. |
| **PR06** · [NHS Couch to 5K](https://www.nhs.uk/better-health/get-active/get-running-with-couch-to-5k/) | condicional | Não presumir direito de copiar app, voz, marca, mídia ou conteúdo de campanhas excluídas; verificar o texto específico e seus termos. | Referência concreta de progressão para corrida; não foi importada. |
...
| **AL04** · [Anses Ciqual 2025](https://doi.org/10.57745/RDMHWY) | candidato | Licence Ouverte Etalab 2.0 identificada no depósito oficial; atribuição e versão necessárias. | 29 linhas de amostra; francês; proteínas por métodos diferentes, limites '<' e valores ausentes. Excel não traz toda a proveniência por nutriente do XML. |
| **AL05** · [CoFID 2021](https://www.gov.uk/government/publications/composition-of-foods-integrated-dataset-cofid) | candidato | Open Government Licence v3.0, observadas exceções declaradas. | 27 linhas selecionadas; números, Tr, N e branco têm sentidos diferentes. Edição 2021 não significa análise nova de cada alimento. |
